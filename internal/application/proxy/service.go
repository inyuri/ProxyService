...
func (s *Service) EvaluateAccess(clientIP, captchaValue string) access.AccessDecision {
	return s.access.Check(clientIP, captchaValue)
}

func (s *Service) AcquireRateLease(clientIP string, uploadHint int64, now time.Time) (*rate_limit.RateLimitLease, *rate_limit.RateLimitViolation) {
	return s.rate.Acquire(clientIP, uploadHint, now)
}

func (s *Service) ReleaseRateLease(lease *rate_limit.RateLimitLease, uploadedBytes, downloadedBytes int64) {
...
func (s *Service) Forward(request ForwardRequest) (ForwardResponse, error) {
	...
	if err != nil {
		s.metrics.UpdateUpstreamStatus(observability.UpstreamStatus{
			Name:      targetURL.Host,
...
func (s *Service) CheckUpstream() {
	...
	status := observability.UpstreamStatus{
		Name:      target.Host,
		URL:       target.String(),
		Healthy:   err == nil,
		LatencyMs: time.Since(startedAt).Milliseconds(),
