...
func (s *Server) telemetryMiddleware() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if leaseValue, ok := c.Get(ctxRateLeaseKey); ok {
			if lease, ok := leaseValue.(*rate_limit.RateLimitLease); ok && lease != nil {
...
func (s *Server) createAccessRuleHandler(c *gin.Context) {
	var request adminapp.AccessRuleRequest
...
func (s *Server) updateRateLimitsHandler(c *gin.Context) {
	var request adminapp.RateLimitUpdateRequest
	if err := c.ShouldBindJSON(&request); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
...
func (s *Server) invalidateCacheHandler(c *gin.Context) {
	var request cache.CacheInvalidationRequest
	if err := c.ShouldBindJSON(&request); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
...
func (s *Server) proxyHandler(c *gin.Context) {
	...
	response, err := s.proxy.Forward(proxyapp.ForwardRequest{
	...
	if err != nil {
		httpErr := &proxyapp.HTTPError{StatusCode: http.StatusBadGateway, Message: "proxy error"}
...
func asProxyError(err error, target *proxyapp.HTTPError) bool {
	value, ok := err.(*proxyapp.HTTPError)
	if !ok {
		return false
	}
	*target = *value
