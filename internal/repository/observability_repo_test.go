...
func TestObservabilityAggregatesEvents(t *testing.T) {
	metrics := NewObservability(domain.ObservabilitySettings{MaxLogs: 10, MaxBuckets: 10})
	metrics.UpdateUpstreamStatus(domain.UpstreamStatus{Name: "httpbin.org", URL: "https://httpbin.org", Healthy: true, LatencyMs: 42})
	metrics.IncActive()
	metrics.Record(domain.RequestEvent{
		Timestamp:     time.Now(),
		IP:            "192.168.1.10",
		Method:        "GET",
		Path:          "/proxy/movies",
