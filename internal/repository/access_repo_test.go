...
func TestAccessServiceCheckPrecedenceAndCaptcha(t *testing.T) {
	...
	err := service.ApplyConfig(domain.AccessSettings{
		DefaultPolicy: "deny",
		CaptchaToken:  "captcha-ok",
		CacheTTL:      time.Minute,
		CacheSize:     64,
		Rules: []domain.AccessRule{
			{ID: "allow-subnet", Type: domain.AccessRuleAllow, Value: "10.0.0.0/24", CreatedAt: time.Now()},
			{ID: "deny-ip", Type: domain.AccessRuleDeny, Value: "10.0.0.5", CreatedAt: time.Now()},
			{ID: "grey-ip", Type: domain.AccessRuleGrey, Value: "192.168.1.50", CreatedAt: time.Now()},
		},
	})
...
func TestAccessServiceSupportsIPv4Ranges(t *testing.T) {
	...
	err := service.ApplyConfig(domain.AccessSettings{
		...
		CacheSize:     64,
		Rules: []domain.AccessRule{
			{ID: "deny-range", Type: domain.AccessRuleDeny, Value: "203.0.113.10-203.0.113.20", CreatedAt: time.Now()},
		},
	})
	require.NoError(t, err)
