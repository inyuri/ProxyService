...
func (c *CacheService) UpdateSettings(settings domain.CacheSettings) {
	c.mu.Lock()
	defer c.mu.Unlock()
...
func (c *CacheService) Get(key string) (*domain.CachedResponse, bool) {
	c.mu.Lock()
	defer c.mu.Unlock()
...
func (c *CacheService) ShouldUse(host, path string, req *http.Request) (domain.CacheRule, bool, string) {
	c.mu.Lock()
	defer c.mu.Unlock()

	if !c.settings.Enabled {
		return domain.CacheRule{}, false, "disabled"
	}
	if req.Method != http.MethodGet && req.Method != http.MethodHead {
		return domain.CacheRule{}, false, "method_not_cacheable"
	}
	if hasNoCacheDirective(req.Header.Get("Cache-Control")) || strings.EqualFold(req.Header.Get("Pragma"), "no-cache") {
		return domain.CacheRule{}, false, "no_cache_request"
	}

	rule := c.resolveRule(host, path)
	for _, header := range rule.SensitiveHeaders {
		if req.Header.Get(header) != "" {
			return domain.CacheRule{}, false, "sensitive_request"
		}
	}
	return rule, true, ""
}
...
func (c *CacheService) Invalidate(req domain.CacheInvalidationRequest) (int, error) {
	c.mu.Lock()
	defer c.mu.Unlock()
...
func (c *CacheService) Stats() domain.CacheStats {
	c.mu.Lock()
	defer c.mu.Unlock()
...
func (c *CacheService) Entries(limit int) []domain.CacheEntryView {
	c.mu.Lock()
	defer c.mu.Unlock()

	result := make([]domain.CacheEntryView, 0, len(c.entries))
	for element := c.order.Front(); element != nil; element = element.Next() {
		...
		result = append(result, domain.CacheEntryView{
			Key:       entry.key,
			Path:      entry.path,
			Status:    entry.status,
			Hits:      entry.hits,
...
func (c *CacheService) Snapshot(limit int) domain.CacheSnapshot {
	c.mu.Lock()
	settings := c.settings
	c.mu.Unlock()

	return domain.CacheSnapshot{
		Stats:   c.Stats(),
		Entries: c.Entries(limit),
		Settings: domain.CacheSnapshotSettings{
...
func (c *CacheService) resolveRule(host, path string) domain.CacheRule {
...
func ttlForStatus(rule domain.CacheRule, status int) time.Duration {
	switch {
